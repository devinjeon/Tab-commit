...
func main() {
	...
	if softMaxLengthEnv := os.Getenv("GPT_COMMIT_MSG_SOFT_MAX_LENGTH"); softMaxLengthEnv != "" {
		softMaxLength = softMaxLengthEnv
	}

	hardMaxLength := defaultHardMaxLength
	if hardMaxLengthEnv := os.Getenv("GPT_COMMIT_MSG_HARD_MAX_LENGTH"); hardMaxLengthEnv != "" {
		hardMaxLength = hardMaxLengthEnv
	}

	language := defaultLanguage
	if languageEnv := os.Getenv("GPT_COMMIT_MSG_LANGUAGE"); languageEnv != "" {
		language = languageEnv
	}

	instructions := defaultInstructions
	if instructionsEnv := os.Getenv("GPT_COMMIT_MSG_INSTRUCTIONS"); instructionsEnv != "" {
		instructions = instructionsEnv
	}

	commitMessageTemplate := defaultCommitMessageTemplate
	if commitMessageTemplateEnv := os.Getenv("GPT_COMMIT_MSG_TEMPLATE"); commitMessageTemplateEnv != "" {
		commitMessageTemplate = commitMessageTemplateEnv
	}

	promptTemplate := defaultPromptTemplate
	if promptTemplateEnv := os.Getenv("GPT_COMMIT_MSG_PROMPT_TEMPLATE"); promptTemplateEnv != "" {
